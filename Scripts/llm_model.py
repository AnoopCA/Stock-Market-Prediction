...
def estimate_loss():
    ...
    for split in ['train', 'val']:
        ...
        for k in range(eval_iters):
            X, Y = get_batch(stock_data, split)
...
if __name__ == "__main__":
    ...
    for iter in range(max_iters):
        if iter % eval_iters == 0:
            ...
            torch.save(model, os.path.join(r'D:\ML_Projects\Stock-Market-Prediction\Models\model_checkpoints', f'model_temp_{iter}_loss-{losses["val"]:.3f}.pth'))

        # sample a batch of data
        xb, yb = get_batch(stock_data, 'train')
        # evaluate the loss
    ...
    torch.save(model, os.path.join(r'D:\ML_Projects\Stock-Market-Prediction\Models', f'model-25_loss-{loss.item():.3f}.pth'))
